$(document).ready(function(){
    ...
    $("#nav_link_quit_hover").hover(function(){
...
//            }
    ...
    // Эта прослушка мешает переходить по ссылкам
    ...
    $('.category_buy_counter > form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.msg == 'false'){
                    $(".item_options .error_msg_counts").html('Выбранное количество товара отсутствует на складе!');
                } else {
                    $(".item_options .error_msg_counts").empty();
    ...
    $('.cart_input_wrap > form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.msg == 'false'){
                    alert('Выбранное количество товара отсутствует на складе!');
                } else {
                    $(".header_cart #amount").html(data.total_amount);
                    $(".header_cart #price").html(data.total_price + " руб.");
                    $(".cart .cart_total_counters").html(data.total_amount);
                    $(".cart .cart_total_price").html(data.total_price + " руб.");

                    var ids = data.id_item;
                    $('td:has(input[value=' + ids + '])').next(" #cart_item_subtotal").html(data.newcount + " руб.");
                }
            },
            error: function(data) {
                console.error("error: " + data);
    ...
    /* Изменение итоговой суммы корзины в зависимости от выбранного способа доставки */
    $('.address_form_field').change(function(){
        selected_value = $("input[name='id_delivery']:checked").val();

        $.ajax({
            url: '/delivery/',
            type: 'post',
            data: {id_delivery : selected_value},
            dataType: 'json',
            success: function(data){
                $('.final_cart_table #delivery_price').html(data.delivery_price + " руб.");
                $('.final_cart_table #final_price').html(data.final_price + " руб.").css("color","#00598c")
                    .css("fontWeight","bold");
            }
        });
    });
    ...
    $('.account_address_delivery > form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.msg == 'success') {
                    alertify.success('Ваши данные успешно обновлены!');
                } else {
    ...
    (function() {
        ...
        $('.logout__link').on('click', function(e) {
            e.preventDefault();
